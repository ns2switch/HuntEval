...
All source references identify exact observable public artifacts. The variants cover trajectory sequences, registered agents, managed actions, tasks, evidence, findings, operational messages, registered metrics, benchmark cells, statistical comparisons, controlled topology experiments, equivalence results, and a safe content-addressed artifact fallback. Readers reject unknown variants and fields. The R5 resolver verifies digest, ownership, run scope, event existence, safe paths, and referential integrity before a writer can emit a diagnosis. Public diagnosis cannot represent ground-truth identifiers, evaluator-only paths or hashes, hidden partitions, raw secrets, or private reasoning.
...
The report stage distinguishes observation, classification, hypothesis, experiment result, and approved change. Schema support for a future stage does not authorize an R5 writer to emit it: R5 hypotheses remain unvalidated and non-adoptable, while controlled validation and human approval belong to R6. Normalized JSON is authoritative; any HTML form must be a deterministic escaped projection. An R5 bundle includes bounded public bytes for every displayed claim source and verifies their exact digests offline; evaluator-only inputs remain excluded. Changing any content-addressed input changes bundle identity and invalidates prior reproduction claims.

## 37. Controlled-improvement contracts v0.8

Schema 0.8 is additive and leaves stored schema 0.3 through 0.7 artifacts unchanged. It defines registered and structured artifacts, registries, safe diffs, improvement policy, paired experiments, equivalence, validation decisions, recommendation lifecycle, human decisions, external adoption records, prompt/configuration weakness taxonomy, normalized reports, and bundle manifests. The schemas define data boundaries; R6-00 does not implement their application services.

A registered artifact binds exact bytes, kind, media type, bounded size, safe label, provenance class, and optional structured representation. A legacy artifact may remain opaque. Structural comparison requires an explicit ordered section inventory with stable section identifiers, exact content hashes, and one policy-owned class. HuntEval never infers section identity from headings or free-form prose. Diff operations can represent only mutable section classes. Exactly one declared experimental artifact variable may change, although every changed section inside that artifact remains visible.

The immutable improvement-policy inventory is fixed to authorization, tool access, filesystem, network, data handling, ground-truth isolation, benchmark constraints, output integrity, and security controls. Candidate bytes cannot change that inventory or reclassify a section. The policy also binds permitted mutable targets, byte and growth limits, answer-leakage checking, regression and verified-resource constraints, mandatory human review, unavailable hidden-test feedback during selection, and prohibited autonomous adoption.

An improvement experiment binds exact baseline, candidate, diff, improvement policy, evaluator-only partition-policy identity, scoring profile, statistical policy, one changed variable, paired cells, and hashes for episode set, seeds, budgets, models, topology, managed-tool policy, execution policy, schemas, and runtime binaries. Partition membership is never part of a public experiment artifact. The candidate is frozen before matrix execution. Equivalence is eligible only when exactly the declared variable changed, all controls are equal, and safety and leakage checks pass.

A validation decision retains paired samples, missing pairs, raw metric deltas, intervals, constraint results, applicability, and limitations. Hidden-test results cannot be used for candidate generation or selection. `passed` supports an experimental `validated` lifecycle transition only for the exact bound bytes. It is not universal causal proof and cannot by itself create approval or adoption.

Recommendation events are append-only and hash-linked. Their deterministic state projection distinguishes `proposed`, `testing`, `validated`, `rejected`, `approved`, `adopted`, `invalidated`, and `superseded`. A validated event requires an exact passing decision reference; approval additionally requires an explicit human-decision reference; adoption additionally requires a separately confirmed external-adoption record. HuntEval records that assertion but never modifies the active deployment.

A prompt recommendation cites exact schema 0.7 observable diagnostic sources, identifies the target agent or deployment and exact artifact section, records a bounded weakness and evidence-sufficiency label, and optionally proposes a typed mutable-section operation. The schema 0.8 prompt/configuration taxonomy is metadata only; executable mappings remain compiled. The canonical taxonomy covers role ambiguity, missing output contract, missing evidence requirements, missing acceptance criteria, missing stopping conditions, unclear tool-use policy, insufficient error handling, insufficient delegation policy, duplicated responsibility, missing task ownership, missing conflict-resolution policy, excessive communication requirements, insufficient evidence-sharing rules, and overly broad specialist invocation criteria.

Normalized JSON remains authoritative and static HTML remains a future safe projection. Improvement bundles contain only bounded safe relative public paths and exact hashes. They exclude ground truth, hidden partition membership and feedback, private paths or hashes, secrets, and private chain of thought. Any changed candidate, baseline, experiment, policy, schema, or relevant binary invalidates the previous validation identity.

## 38. Knowledge and extension contracts v0.9

Schema 0.9 is additive and leaves schemas 0.3 through 0.8 unchanged. An analytical corpus declares exactly one authorization scope and an ordered inventory of successfully verified public source identities, kinds, and SHA-256 digests. `evaluator_analytics` accepts supported verified HuntEval outputs but can never be mounted or queried through an evaluated deployment. `deployment_visible` accepts authored public documents only and remains optional, untrusted, network-free, and disabled by default.

Index construction requires exact agreement with corpus membership and bounded normalized field maps. Corpus and index identities derive from canonical bytes. Structured queries bind an exact index and scope, contain at most sixteen bounded terms, optionally restrict source kinds, and cap results. Each match records exact source identity, kind, artifact hash, field, and escaped excerpt. Results do not create missing metrics, causal claims, experimental validation, approval, or adoption. Retrieval audit events bind query, result, index, scope, UTC time, measured latency, explicit cost availability, sequence, and the previous event hash.

An extension manifest declares whether an out-of-process executable is a managed-tool or deployment adapter, its exact binary digest, supported contract versions, requested capabilities, denied network policy, bounded tool inventory, and positive resource limits. The manifest is a request, not authorization. A runner-owned capability policy resolves the request deny-by-default and grants nothing when any capability or limit is unsupported. Managed-tool adapters use a bounded one-request/one-response schema 0.9 process contract with exact correlation identity; deployment adapters use protocol 0.3. Conformance binds exact manifest, executable, policy, and behavioral transcript bytes. Scored tools remain runner-mediated and scored network access remains denied.

The pure Python SDK consumes the same canonical public fixtures. Its initial typed schema support is 0.9 and its deployment-side JSONL peer targets protocol 0.3; older public JSON artifacts can be read as digest-verified opaque objects but are not advertised as typed schema support. It provides strict models, root-confined digest-verifying local readers, and a bounded deployment-side peer. It does not contain private artifact models and has no evaluation, scoring, authoritative verification, orchestration, provider, or direct-tool authority. The SDK compatibility index independently versions the package, Python floor, supported schemas/protocols, and a fixture inventory that includes the content-addressed protocol compatibility manifest.

## 39. Release-candidate contracts v1.0

Schema 1.0 adds release governance and does not relabel or modify schemas 0.3 through 0.9. The interface inventory records owner, stability, version range, public/private projection, authority, trust boundary, bounds, parser behavior, precondition, fixture, verification gate, and limitations. Only public, documented, fixture-backed interfaces with satisfied preconditions can enter the deterministic freeze manifest. Pending connector interfaces remain excluded.

The compatibility matrix binds the exact freeze inventory and declares complete component combinations as `supported`, `retained`, `preview`, or `unavailable`. Supported and retained rules may reference only frozen interfaces. Preview and unavailable rules require typed reasons and limitations. Missing combinations are unavailable. Compatibility grants no runtime, sandbox, network, tool, signing, publication, or quality authority.

The migration inventory declares only retained `adapt_in_memory`, `read_as_is`, and `reject` behavior. Source artifacts remain immutable. A receipt binds exact source and target hashes, edge identity, and implementation. Undeclared, ambiguous, downgrade, future-major, lossy, and private/public transitions fail closed.

Package inventory, build provenance, SPDX SBOM, dependency/license evidence, release manifest, detached signature policy/inventory, official benchmark-pack root, native platform matrix/evidence, and R8 evidence index are bounded public contracts. The platform matrix binds each target to one OS, architecture, native runner, archive format, binary suffix, support level, sandbox state, and validation state. macOS and Windows preview packages cannot claim scored execution, and missing native evidence remains unavailable. Provenance and signatures are evidence, not authorization. The closure verifier requires every milestone, pre-R8 dependency decision, independent review, protected check, immutable rehearsal, artifact identity, and limitation; missing evidence cannot produce a passing result.
