...
The token must be supplied through the process environment, never stored in the repository or command output. The verifier fails closed if required checks, approvals, CODEOWNER enforcement, administrator enforcement, force-push prevention, deletion prevention, or an active protected-tag ruleset cannot be established.
...
Stop new merges and releases, preserve failed workflow logs within their retention window, and open a private security report when confidentiality or integrity may be affected. Revert the smallest offending commit through a reviewed pull request. Do not rewrite `main` or an existing release tag. Invalidate compromised tokens, rebuild on a clean runner with empty caches, rerun all canonical scripts, and publish corrected checksums under a new release-candidate tag. Production rollback remains a maintainer decision and must reference the affected and replacement revisions.
