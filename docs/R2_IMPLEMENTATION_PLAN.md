...
| R2-17 | complete | `5874792`, `23deef2`, `5a38c63` — least-privilege GitHub Actions jobs, pinned sandbox runner, image-bound caches, bounded artifacts, and clean-cache parity; run `31255365813` passed |
...
Implementation evidence: commit `5874792` pins Rust 1.93.1 and cargo-deny 0.20.2; adds canonical fail-closed quality, security, end-to-end, cache-parity, settings-verification, evidence, and RC scripts; and makes seven least-privilege GitHub Actions jobs consume those entrypoints. Local policy, formatting, lint, tests, documentation, dependency, architecture, source-size, security, negative-propagation, clean-cache parity, and 36-cell end-to-end gates pass. The clean-tree RC dry run produced `hunteval-rc-5874792e4e5a-x86_64-unknown-linux-gnu.tar.gz` and a verified SHA-256 manifest without publishing a release. Remote runs exposed incompatible Bubblewrap behavior behind the moving `ubuntu-latest` label and a native target cache shared across runner images; commits `23deef2` and `5a38c63` pin Ubuntu 22.04 and bind cache namespaces to that image. GitHub Actions run `31255365813` then passed Policy, Quality, Tests, Security, End-to-end, Documentation, and Package on the same revision. At that revision, R2-18 and the R2.4 exit gate remained pending until an authorized maintainer completed `GITHUB_SETTINGS_ATTESTATION.md` and a protected RC tag ran successfully.

External closure evidence: administrator `ns2switch` completed the live-settings attestation on 2026-08-09. Protected `main` requires the nine current jobs, review controls, administrator enforcement, and conversation resolution; active `v*` rulesets restrict creation and make update/deletion non-bypassable. CI run `31322660682` passed on revision `b412953a08f3e2e26dff82c1aa0a729515496564`; protected tag `v0.4.0-rc.1` references that revision; and release-candidate run `31329216944` passed with independently verified package and secret-scan hashes and no production release. Exact evidence is recorded in `GITHUB_SETTINGS_ATTESTATION.md`. R2-18 and the R2.4 exit gate are complete.
